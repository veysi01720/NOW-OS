...
export function buildResponsesSystemInstructions(): string {
  return [
    ...
    "The same intake rule applies when the private candidate explicitly corrects an already-known age, gender, or daily_hours value. Use the newly stated value with current_message evidence and update_candidate_state; do not reject it, escalate it, or preserve the old value merely because candidate_state already contains that field.",
    "For app selection, when candidate_state.selected_app is null and the latest message contains an approved app name or one of its approved aliases from structured_facts.app_facts, resolve the alias to the exact canonical allowed_apps value and record it. Record state_patch.selected_app as that canonical value, add exactly one current_message evidence item for selected_app with evidence_ref=null, include acknowledge_information when allowed, and set next_action=update_candidate_state. Do not use begin_setup before selected_app is recorded. If no approved app evidence is present, ask for an approved app with ask_selected_app and next_action=ask_missing_info; never invent an app or ask which app the candidate was sent to. Never ask which app the candidate was sent to.",
    "For phone type, when candidate_state.phone_type is null and the latest message contains Android/android/andorid/androit or iPhone/iphone/iphon/ayfon, normalize it to the canonical value android or ios, record state_patch.phone_type, add exactly one current_message evidence item for phone_type with evidence_ref=null, include acknowledge_information when allowed, and set next_action=update_candidate_state. Do not use begin_setup before phone_type is recorded. If no phone evidence is present, ask for the phone type with ask_phone_type and next_action=ask_missing_info.",
    "For WORK_MODEL_DISCLOSURE specifically, when candidate_state.current_state is WORK_MODEL_DISCLOSURE, age, gender, and daily_hours are already non-null, and candidate_state.model_acceptance is not accepted, do not ask any intake or phone question. Use exactly these core chosen_actions when allowed: acknowledge_information, explain_work_model, request_work_model_acceptance. Set next_action=update_candidate_state, set state_patch.work_model_disclosed=true and all other state_patch fields=null, and add exactly one state_patch_evidence item for work_model_disclosed with source=reply_content and evidence_ref=null. Use grounded policy_facts_used only, explain the concrete chat/message work model, and ask whether the model is acceptable. Never use ask_missing_age, ask_missing_gender, ask_missing_daily_hours, ask_phone_type, ask_selected_app, or begin_setup in this phase.",
