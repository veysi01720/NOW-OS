...
const FORBIDDEN = [
  ...
  "sahte kimlik", "izinsiz", "yasak", "uydur", "iddia edemem", "dogrulanmis bilgi bulunmuyor"
];
...
export const golden_conversation_pack_v1: CertScenario[] = [
  ...
  { id: "cert_002", category: "single_message_intake", turns: ["27 erkek 4"], requiredState: { age: 27, gender: "erkek", daily_hours: 4 }, requiredReplyIncludes: ["mesaj"] },
  { id: "cert_003", category: "single_message_intake", turns: ["27 e 4"], requiredState: { age: 27 }, requiredReplyIncludes: ["cinsiyet", "saat"] },
  { id: "cert_004", category: "single_message_intake", turns: ["27 yas erkegim dort saat"], requiredState: { age: 27, gender: "erkek", daily_hours: 4 }, requiredReplyIncludes: ["mesaj"] },
  { id: "cert_005", category: "single_message_intake", turns: ["27 yas erkek 4 saat"], requiredState: { age: 27, gender: "erkek", daily_hours: 4 }, requiredReplyIncludes: ["mesaj"] },
  { id: "cert_006", category: "single_message_intake", turns: ["27,erkek,gunde4"], requiredState: { age: 27, gender: "erkek", daily_hours: 4 }, requiredReplyIncludes: ["mesaj"] },
  { id: "cert_007", category: "single_message_intake", turns: ["erkek 27 gunluk dort saat"], requiredState: { age: 27, gender: "erkek", daily_hours: 4 }, requiredReplyIncludes: ["mesaj"] },
  { id: "cert_008", category: "single_message_intake", turns: ["yasim 27 erkegim 4 saat ayiririm"], requiredState: { age: 27, gender: "erkek", daily_hours: 4 }, requiredReplyIncludes: ["mesaj"] },
  { id: "cert_009", category: "single_message_intake", turns: ["27 erkek yaklasik 4-5 saat"], requiredState: { age: 27, gender: "erkek", daily_hours: 4 }, requiredReplyIncludes: ["mesaj"] },
  { id: "cert_010", category: "partial_intake", turns: ["Selam", "27", "erkek", "4 saat"], requiredState: { age: 27, gender: "erkek", daily_hours: 4 }, requiredReplyIncludes: ["mesaj"] },
  ...
  { id: "cert_021", category: "male_account_question", turns: ["27 erkek 4", "Erkek hesabi mi acacagiz?"], requiredReplyIncludes: ["kadin", "profil"] },
  { id: "cert_022", category: "male_account_question", turns: ["27 erkek 4", "erkek profil mi gerekiyor"], requiredReplyIncludes: ["kadin", "profil"] },
  ...
  { id: "cert_036", category: "state_correction", turns: ["27", "erkek 4 saat"], requiredState: { age: 27, gender: "erkek", daily_hours: 4 }, requiredReplyIncludes: ["mesaj"] },
  { id: "cert_037", category: "state_correction", turns: ["erkek", "27 4 saat"], requiredState: { age: 27, gender: "erkek", daily_hours: 4 }, requiredReplyIncludes: ["mesaj"] },
  { id: "cert_038", category: "state_correction", turns: ["4 saat", "27 erkek"], requiredState: { age: 27, gender: "erkek", daily_hours: 4 }, requiredReplyIncludes: ["mesaj"] },
  { id: "cert_039", category: "payment_question", turns: ["27 erkek 4", "para isi nasil oluyor"], requiredReplyIncludes: ["kazanc", "performans"] },
  { id: "cert_040", category: "trust_question", turns: ["27 erkek 4", "guvenli mi"], requiredReplyIncludes: ["surec", "kontrol"], forbiddenReplyPhrases: ["kesin garanti"] },
...
class CertificationAssistantClient implements AssistantClient {
  ...
  private replyForContext(context: any): string {
    ...
    if (latest.includes("erkek hes") || latest.includes("erkek profil")) return decision({ text: "Erkek adaylarda calisma kadin profili acilmasi ve uygun kadin fotograflari kullanilmasi uzerinden ilerler; bu model sana uygunsa acik onayinla devam ederiz.", intent: "account_profile_question", actions: ["answer_user_question"], facts: facts.includes("male_candidate_work_model") ? ["male_candidate_work_model"] : [], direct: true });
    if (/(anlamadim|nasil yani|daha basit|tam anlamadim|ne demek|ornek|mesajlasma nasil|kamera acacak|kisa anlat|anlat dedim)/u.test(latest)) return decision({ text: latest.includes("kamera") ? "Kamera tarafinda dogrulanmis zorunluluk soylemiyoruz. Basitce aday uygulamada mesajlara yaziyla cevap vererek ilerler." : latest.includes("ornek") ? "Ornek olarak: uygulamada sana gelen mesaja kisa ve duzgun cevap verirsin; ekip de hangi adimda ne yapacagini soyler." : latest.includes("kisa") ? "Kisaca: calisma modeli, onayli uygulamada mesajlasma uzerinden yaziyla cevap vermektir." : "Daha basit anlatayim: onayli uygulamada sohbetlere yaziyla cevap vererek ilerlersin. Once bu mesajlasma agirlikli modelin sana uyup uymadigini netlestiriyoruz.", intent: "clarify_previous_explanation", actions: ["answer_user_question", "clarify_previous_explanation", "explain_work_model"], facts: facts.filter((id: string) => ["male_candidate_work_model", "candidate_work_steps_chat_based"].includes(id)), direct: true });
    if (/(nasil yapacagim|bu isi nasil|bu is tam|tam olarak ne|kamera|yazisma|guvenli|para isi|kamerasiz)/u.test(latest)) return decision({ text: latest.includes("guvenli") || latest.includes("para") ? "Surec uygulama ekranlari ve ekip yonlendirmesiyle kontrol edilerek ilerler; kazanc performansa ve uygulama surecine gore degisir." : latest.includes("kamera") || latest.includes("yazisma") || latest.includes("kamerasiz") ? "Kamera zorunlu degil diye anlatmiyoruz; mesajlasma agirlikli ilerlemek isteyen aday icin onayli uygulama secilir." : "Onayli uygulamada sohbetlere yaziyla cevap vererek ilerlersin. Kurulumdan once bu calisma modelinin sana uygun oldugunu netlestirelim.", intent: context.latest_message.inferred_intent === "ask_job_definition" ? "ask_job_definition" : "ask_how_work_is_done", actions: ["answer_user_question", "explain_work_model", "request_work_model_acceptance"], statePatch: { work_model_disclosed: true, work_model_acceptance: "pending" }, facts: facts.filter((id: string) => ["male_candidate_work_model", "work_model_acceptance_required", "candidate_work_steps_chat_based"].includes(id)), nextAction: "request_work_model_acceptance", direct: true });
    ...
    if (latest.includes("sinirlendim") || latest.includes("olmuyor")) return decision({ text: "Haklisin, sade anlatayim: onayli uygulamada yaziyla sohbetlere cevap verirsin. Erkek adaylarda kadin profili acilmasi ve kadin fotograflari kullanilmasi uzerinden ilerler; adimlari tek tek netlestirecegim.", intent: "clarify_previous_explanation", actions: ["answer_user_question", "explain_work_model"], direct: true, facts: facts.filter((id: string) => ["male_candidate_work_model", "candidate_work_steps_chat_based"].includes(id)) });
    ...
    return decision({ text: "Onayli uygulamada mesajlara yaziyla cevap vererek ilerlersin; once bu calisma modelinin sana uygun oldugunu netlestirelim.", intent: "candidate_next_step", actions: ["answer_user_question", "explain_work_model", "request_work_model_acceptance"], statePatch: { work_model_disclosed: true, work_model_acceptance: "pending" }, facts: facts.filter((id: string) => ["male_candidate_work_model", "work_model_acceptance_required"].includes(id)), nextAction: "request_work_model_acceptance" });
  }
}
...
describe("Conversation Decision V2 final certification pack", () => {
  ...
  it("passes all golden scenarios through the real candidate V2 route with noop outbound", async () => {
    ...
    expect(modelOrigin).toBeGreaterThan(0);
