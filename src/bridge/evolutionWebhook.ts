...
import { handleIncomingMessage, type HandleIncomingMessageDeps } from "./handleIncomingMessage.js";
...
export function registerEvolutionWebhook(app: FastifyInstance, deps: HandleIncomingMessageDeps): void {
  app.post("/webhooks/evolution", async (request, reply) => {
    ...
    try {
      ...
      const normalized = normalizeEvolutionMessage(request.body);
      ...
      if (normalized.message_id.trim() === "") {
        ...
        return reply.code(200).send({ status: "ignored", reason: "missing_provider_message_id" });
      }
