...
export type InstallationVerificationReasonCode =
  | "INSTALLATION_SCREEN_CONFIRMED"
  | "INSTALLATION_SCREEN_UNCLEAR"
  | "MEDIA_BYTES_MISSING"
  | "MEDIA_BYTES_EMPTY"
  | "MEDIA_SIZE_EXCEEDED"
  | "VISION_CLASSIFIER_NOT_CONFIGURED"
...
export interface InstallationVerificationClassifierResult {
  ...
  reason_code?: InstallationVerificationReasonCode;
...
function ambiguousResult(
  ...
  buffer: Buffer,
  now: number,
  reason: InstallationVerificationReasonCode,
...
export async function verifyInstallationMedia(input: {
  ...
  const classified = classifier
    ...
    : { status: "ambiguous" as const, reason_code: "VISION_CLASSIFIER_NOT_CONFIGURED" as const, sanitized_result: "VISION_CLASSIFIER_NOT_CONFIGURED" };
  ...
  return {
    ...
    reason_code: classified.reason_code ?? (classified.status === "clear" ? "INSTALLATION_SCREEN_CONFIRMED" : "INSTALLATION_SCREEN_UNCLEAR"),
